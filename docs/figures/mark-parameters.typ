  ...
  box(inset: (x: 1em, y: .5em, bottom: 1em), box(
    width: len, height: wid, stroke: .1pt + luma(if dark {60%} else {0%}),
    {
      ...
      mark.map(mark => {
        ...
        place(dy: 50%, dx: len - mark.end, place(horizon, line(angle: 90deg, stroke: .4pt + red)))
      }).join()
      annotations.pos().join()
      if length != auto {
        place(dy:  - .1em, dx: len/2, place(center+bottom)[`length`])
      }
      place(dy: wid/2, dx: len + .1em, place(horizon + left, rotate(90deg, reflow: true, origin: left)[`width`]))

      set text(gray, 1.2em)
      place(
        bottom + center, dy: 2pt,
...
#describe-mark(
  ...
  place({
    ...
    place(arc(origin: (0pt, y), angle: 90deg, arc: -140deg, radius: 20pt, stroke: .5pt))
    place(dx: 2pt, dy: .76cm)[`arc`]
  }),
  name: "hooks"
)
#describe-mark(
  width: 3cm, length: 1cm,
  bracket,
  name: "bracket"
)
#describe-mark(
...
#describe-mark(
  wid: 3cm, length: 1.5cm, len: 3cm,
  rays.with(n: 6, align: end),
  name: "rays"
)

#describe-mark(
  width: 3cm, length: 2cm,
